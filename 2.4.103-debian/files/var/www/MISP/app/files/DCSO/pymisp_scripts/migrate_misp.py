...
def migrate_events(START_EVENT_ID,END_EVENT_ID, misp_new, misp_old, sql_handler):
    ...
    for EVENT_ID in range(START_EVENT_ID, END_EVENT_ID+1):
        ...
        try:
            print(str(START_TIME) + ': ' + str(EVENT_ID)+' / '+ str(END_EVENT_ID)+' events migrated...New ID: '+ NEW_EVENT['Event']['id'] + '...UUID: ' + NEW_EVENT['Event']['uuid']+'...Info:' + NEW_EVENT['Event']['info'])
        except:
            #print('NEW_EVENT EXCEPT')
